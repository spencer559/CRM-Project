...
> This README doubles as a **project handoff / context document** — it captures the architecture, conventions, and the vendor-specific gotchas that took real reports to discover. **AI coding agents** (Claude Code, Codex/ChatGPT, or anything else) start from [`AGENTS.md`](AGENTS.md), the short working brief — commands, architecture, rules that are easy to break — which points back into the relevant sections here. `CLAUDE.md` just imports it.
