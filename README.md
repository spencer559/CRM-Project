...
- **Node tests:** `npm test` (or `node tests/run.js`) runs the whole suite — 61 files, about 5 s. The
  ...
  | `crmdb-webkit-idb-slices` | on WebKit the working copy survives an app restart: IndexedDB is handed whole parts, never slices, and what it hands back is copied before the next save deletes it. Models both WebKit bugs, since Node has neither |
